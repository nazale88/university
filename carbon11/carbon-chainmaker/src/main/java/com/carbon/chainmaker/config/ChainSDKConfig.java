...
public class ChainSDKConfig {
    ...
    public ChainClient chainClient() throws IOException, SdkException {
        ...
        ChainClientConfig chainClient1 = sdkConfig.getChainClient();
        String userCrtFilePath = chainClient1.getUserCrtFilePath();
        byte[] resourceFileBytes = this.getResourceFileBytes(userCrtFilePath);
        ChainClient chainClient = chainManager.getChainClient(sdkConfig.getChainClient().getChain_id());
        User clientUser = new User(sdkConfig.getChainClient().getOrgId(), this.getResourceFileBytes(sdkConfig.getChainClient().getUserKeyFilePath()), resourceFileBytes);
        if (chainClient == null) {
