...
//        String key = RedisKeyName.SMS_UPDATE_PHONE_KEY + email;
...
//
//        sendMsg(email, templateCode, JSONUtil.toJsonStr(param));
//    }
     @Override
     public void sendUpdatePhoneCodeByEmail(Long accountId) {
         ...
         // 生成验证码
         String validCode = RandomUtil.randomNumbers(6);
         String key = RedisKeyName.SMS_UPDATE_PHONE_KEY + account.getEmail(); // 构造Redis存储key
//         // 准备验证码参数
//         HashMap<String, Object> param = new HashMap<>();
//         param.put("code", validCode);
...
//
//        if (StrUtil.isBlank(account.getEmail())) {
//            throw new CommonBizException("账户未绑定邮箱");
//        }
        //校验短信验证码
        checkValidateCode(account.getEmail(),param.getCode(),RedisKeyName.SMS_UPDATE_PHONE_KEY);
        //查询该手机是否已绑定别的账号
    ...
    public void sendEmail(SendEmailParam param) {
        ...
        String text="点击以下链接完成邮箱验证\n" +
                emailBaseUrl +"/system/sysAccount/renew/email/"+param.getId().toString()+"/"+param.getEmail()+"/"+value;
...
//        超管账户不可删除
    ...
    public void checkValidateCode(String phone, String code, String type) {
        ...
        if (StrUtil.isEmpty(redisMsgCode)) {  //验证码过期了就不存在了
            throw new CommonBizException("验证码已过期");
        }
        if (!code.equals(redisMsgCode)) {
            throw new CommonBizException("请输入正确的验证码");
        }
        //验证码消费后失效， Hutool 工具类，CollUtil.newArrayList() 是 Hutool 提供的工具方法，专门用于快速创建 ArrayList：
        redisService.delete(CollUtil.newArrayList(redisKey));//批量删除
    }
}
