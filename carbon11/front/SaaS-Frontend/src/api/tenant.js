...
export function getUpLoadFileParams() {
    ...
    return {
       // url:'http://localhost:9002/system/oss/uploadImages'
...
export function getUpLoadProjectParams() {
    let token = !store.getters.token ? sessionStorage.getItem('token') : store.getters.token;
    return {
      // url:'http://localhost:9002/system/oss/uploadFiles'
...
export function getFeiShuUpLoadProjectParams() {
    ...
    return {
        ...
        // url: `${SettingMer.apiBaseURL}/assets/exchangeAccount/uploadCredential`, 指向9001
        token: token
    }
}
// 飞书项目文件上传
export function getFeiShuUpLoadFileParams() {
    let token = !store.getters.token ? sessionStorage.getItem('token') : store.getters.token;
    return {
      url:'http://localhost:9002/system/feishu/uploadFile',
