<template>
  <div class="divBox">
    <el-card class="box-card" v-loading="loadingAll">
      <el-alert :closable="false" title="你可用该标题的模板搭配不同的关键词使用，配置提交后关键词种类和顺序将不能修改" type="warning">
      </el-alert>
            ...
            <el-form ref="form" :model="form" :rules="rules" label-width="100px" size="mini">
              ...
              <el-form-item :label=" '已选择(' + KeywordCheck.length + '/5)'">
                ...
                <div v-else class="tmplmsg-form-check">
                  <div class="tmplmsg-form-check-list mb10 acea-row row-between" v-for="(item, index) in KeywordCheck" :key="item.kid" draggable="true" @dragstart="handleDragStart($event, item)" @dragover.prevent="handleDragOver($event, item)" @dragover="handleDragEnter($event, item)" @dragend="handleDragEnd($event, item)">
              ...
              <el-form-item label="场景说明" prop="sceneDesc">
                <el-input type="textarea" :rows="2" placeholder="请输入场景说明" v-model="form.sceneDesc">
                </el-input>
              </el-form-item>
              <el-form-item>
